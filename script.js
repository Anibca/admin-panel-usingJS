...
function renderUsers(users, isFirstTime, searchValue) {
  ...
  users.map((user, i) => {
    tableBody.innerHTML += `
    <tr class="data-row ${
      (isFirstTime && i === 2) || user.id === activeUserId ? "active" : ""
    }" onclick="userRowClicked(this)">
      <td class="column1">${user.id}</td>
      <td class="column2">${user.firstName.replace(
        reg,
        (str) => "<span>" + str + "</span>"
      )}</td>
      <td class="column3">${user.lastName.replace(
        reg,
        (str) => "<span>" + str + "</span>"
      )}}</td>
      <td class="column4">${user.email.replace(
        reg,
        (str) => "<span>" + str + "</span>"
      )}}</td>
      <td class="column5">${user.phone.replace(
        reg,
        (str) => "<span>" + str + "</span>"
      )}}</td>
    </tr>`;
...
function userRowClicked(clickedRow) {
  ...
  const clickedUserId = (activeUserId =
    +clickedRow.querySelector(".column1").innerHTML);
